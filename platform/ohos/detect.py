...
def configure(env: "SConsEnvironment"):
    ...
    # 链接库：OHOS 系统库（native_window / vulkan / hilog_ndk / ace_ndk / z / dl / pthread）
    # 注意：hilog_ndk、ace_ndk 与 ace_napi 在 OHOS SDK 中命名为 *.z.so（ELF 格式），
    # 需用 -l:精确文件名 链接；其余为标准 .so/.a。
    env.Append(LIBS=["native_window", "vulkan", "z", "dl", "pthread"])
    env.Append(LINKFLAGS=["-l:libhilog_ndk.z.so", "-l:libace_ndk.z.so", "-l:libace_napi.z.so"])
